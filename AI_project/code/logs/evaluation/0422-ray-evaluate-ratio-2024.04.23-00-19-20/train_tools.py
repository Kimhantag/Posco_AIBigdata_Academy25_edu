...
class train_tools(object):
    def __init__(self, writer, timeStr, PCT_policy, args):
        ...
        elif self.use_acktr == 'ppo':
            self.policy_optim = PPO(self.PCT_policy, clip_param=args.clip_param, ppo_epoch=args.ppo_epoch, num_mini_batch=args.num_mini_batch,
                                    value_loss_coef=args.value_loss_coef, entropy_coef=args.entropy_coef, lr=args.learning_rate, eps=args.evaluation_episodes,
                                    max_grad_norm=args.max_grad_norm)
        elif self.use_acktr=='a3c':
            self.policy_optim = A3C(self.PCT_policy, 
                                    clip_param=args.clip_param,
                                    max_grad_norm=args.max_grad_norm,
                                    lr=args.learning_rate, 
                                    eps=args.evaluation_episodes)
        elif self.use_acktr =='A2C':
            self.policy_optim = optim.Adam(self.PCT_policy.parameters(), lr=args.learning_rate)
        elif self.use_acktr == 'policy_gradient':
            self.policy_optim = PolicyGradient(self.PCT_policy, lr=args.learning_rate, 
                                               eps=args.evaluation_episodes, max_grad_norm=args.max_grad_norm)

        elif self.use_acktr == 'trpo':
            self.policy_optim = TRPO(self.PCT_policy,
                                     max_kl=args.max_kl,
    ...
    def train_n_steps(self, envs, args, device):
        ...
        while True:
            ...
            if self.use_acktr == 'acktr' and self.policy_optim.steps % self.policy_optim.Ts == 0:
            ...
            # For 'acktr' and 'A2C' update
            if self.use_acktr == 'acktr' or self.use_acktr == 'A2C':
                print('###########', self.use_acktr, '###########')
            ...
            # For other algorithms (e.g., 'ppo', 'a3c', 'policy_gradient', 'trpo')
            else:
                print('###########', self.use_acktr, '###########')
                self.policy_optim.zero_grad()
                (args.actor_loss_coef * actor_loss
