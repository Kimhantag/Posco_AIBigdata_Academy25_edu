...
class PackingContinuous(gym.Env):
    ...
    def step(self, action):
        ...
        succeeded , z= self.space.drop_box(next_box, idx, rotation_flag, self.next_den, self.setting) #z 추가함

        if not succeeded:
            reward = 0.0
            done = True
            info = {'counter': len(self.space.boxes), 'ratio': self.space.get_ratio(), 'density' : self.space.get_density,
                    'reward': self.space.get_ratio() * 10 + 0.007 * self.space.get_density()}
