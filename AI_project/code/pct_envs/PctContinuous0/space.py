...
#packing된 item의 총 density 를 하다 말았다는 전 기수의 부분
...
#packing된 item의 총 mass
    ...
    def drop_box(self, box_size, idx, flag, density, setting, **kwargs): #drop_box 함수 수정함
        if not flag:
            x, y, z = box_size
        else:
            y, x, z = box_size
    ...
    def check_box(self, max_h, box_now, setting, virtual=False):
        assert isinstance(setting, int), 'The environment setting should be an integer.'
