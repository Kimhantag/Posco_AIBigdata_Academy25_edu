...
class PackingDiscrete(gym.Env):
    ...
    # 밀도 저장
    def get_box_denstiy(self):
        density = self.next_den
        return density
    ...
    def cur_observation(self):
        ...
        if self.test:
            ...
            if self.setting < 3: self.next_den = 1
                ...
                self.next_den = round(np.random.random(),2)
    ...
    def step(self, action):
        ...
        succeeded , z= self.space.drop_box(next_box, idx, rotation_flag, self.next_den, self.setting)

        if not succeeded:
            #print('##### 한 상자 종료',self.space.get_ratio() * 10)
            
            done = True
            info = {'counter': len(self.space.boxes),
                    'ratio': self.space.get_ratio(),
                    'density' : self.space.get_density,
                    'reward': self.space.get_ratio() * 10 + 0.01 * self.space.get_density()}
            reward = 0.0
        ...
        self.packed.append(
            [packed_box.x, packed_box.y, packed_box.z,
             ...
             packed_box.density, packed_box.destination_id, packed_box.density])

        box_ratio = self.get_box_ratio()
        box_density = self.get_box_denstiy()
        self.box_creator.drop_box()  # remove current box from the list
        self.box_creator.generate_box_size()  # add a new box to the list

        # scaling 시도
        #min_value = 0, max_value = box_density * bin_size_2
#         reward = box_ratio * 10 + 0.01 * min_max_scaling(box_density * (self.bin_size[2] - z),
#                                                          0,
#                                                          box_density * self.bin_size[2]) 
        ...
        reward += 10*destination_reward  # 최종 보상에 배송지 번호 보상 추가
