...
class Box(object):
    def __init__(self, x, y, z, lx, ly, lz, density, virtual=False, destination_id=None):
        self.x = x
        self.y = y
        self.z = z
        self.lx = lx
        ...
        #배송지 번호 추가
        self.destination_id = destination_id if destination_id else random.randint(1, 11)
    ...
    # 배송지 번호에 따른 이상적인 z 좌표 계산
    # bin3D의 배송지 보상함수랑 중복됨
    #def calculate_reward(self, max_height):
        #ideal_z = max_height / 2
        #reward = max(0, 1 - abs(ideal_z - self.z) / max_height)
        #return reward
...
#packing된 item의 총 density 를 하다 말았어요ㅎㅎㅎㅎ
    def get_density(self):
        do = reduce(lambda x, y: x + y, [box[6]*(self.height-box[2]) for box in self.box_vec], 0.0)
        return do

#packing된 item의 총 mass
    def get_mass(self):
        do = reduce(lambda x, y: x + y, [box.density for box in self.boxes], 0.0)
        vo = reduce(lambda x, y: x + y, [box.x * box.y * box.z for box in self.boxes], 0.0)
        mo = do * vo
        return mo
    ...
    def drop_box(self, box_size, idx, flag, density, setting):
        ...
        if sta_flag:
#             reward = box_now.calculate_reward(self.height)  # bin3D에서 배송지 보상함수가 중복되어 return에 reward 지움
...
#         elif setting == 3:
#             if max_h > 130:
#                 if box_now.density > 0.8:
#                     return False
#                 elif box_now.density > 0.8 and box_now.mass > 28000:
#                     return False
#                 elif box_now.density > 0.7 and box_now.mass > 43000:
#                     return False
#             elif max_h > 100:
#                 if box_now.density > 0.8:
#                     return False
#                 elif box_now.density > 0.7 and box_now.mass > 24000:
#                     return False
#                 elif box_now.density > 0.6 and box_now.mass > 37000:
#                     return False
#             elif max_h > 95:
#                 if box_now.density > 0.7:
#                     return False
#                 elif box_now.density > 0.6 and box_now.mass > 29000:
#                     return False
#                 elif box_now.density > 0.5 and box_now.mass > 30000:
#                     return False
        else:
            if max_h == 0:
                return True
